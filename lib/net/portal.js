...
const BufferProxy = require('./proxy/buffer-proxy');
const EditorProxy = require('./proxy/editor-proxy');
const EditorProxyMetadata = require('./proxy/editor-proxy-metadata');
...
const {deserializeOperation} = require('@atom/teletype-crdt');


module.exports =
  class Portal {
    constructor({id, hostPeerId, siteId, decNet}) {
      this.id = id;
      ...
      if (this.isHost) {
        this.bindPeerIdToSiteId(this.decNet.userId, this.siteId);
        this.nextSiteId = 2;
        this.nextBufferId = 1;
        this.nextEditorId = 1;
        this.disposables.add(
          this.decNet.onRequest(`/portals/${this.id}`, this.hostPeerId, this.id, this.receiveSubscription.bind(this))
        )
      } else {
      ...
      this.disposables.add(
        this.decNet.onNotification(`/portals/${this.id}`, this.hostPeerId, this.id, this.receiveUpdate.bind(this)),
        this.decNet.onMemberLeave(this.siteDidLeave.bind(this))
      )
    }
    ...
    // async initialize() {
    //   try {
    //     this.disposables.add(await this.decNet.listen())
    //   } catch (error) {
    //     this.dispose();
    //     throw error
    //   }
    ...
    async join() {
      // Request a remote Portal Subscription and unpack the response
      const requestResponse = await this.decNet.request({
      ...
      if (!requestResponse.ok) {
        throw new Error("Request to join was unsuccessful")
      }
      const dmResponse = Messages.DM.deserializeBinary(requestResponse.body);
      const response = dmResponse.getPortal().getSubscriptionResponse();

      response.getSiteIdsByPeerIdMap().forEach((siteId, peerId) => {
        this.bindPeerIdToSiteId(peerId, siteId)
      });
      this.siteId = this.siteIdsByPeerId.get(this.decNet.userId);
    ...
    async fetchBufferProxy(id) {
      const response = await this.decNet.request({
        hostId: this.hostPeerId,
        portalId: this.id,
        channelId: `/buffers/${id}`
      });
      if (response.ok) {
        const bufferProxyMessage = Messages.DM.Portal.BufferProxy.deserializeBinary(response.body);
    ...
    async fetchEditorProxy(id) {
      ...
      const editorProxyMessage = Messages.DM.Portal.EditorProxy.deserializeBinary(response.body);
    ...
    getActiveSiteIds() {
      const connectedMemberIds = this.decNet.getMemberIds();
      const activeSiteIds = [];
    ...
    getSiteIdentity(siteId) {
      ...
      // TODO: Insert aliases
      // return this.decNet.getMemberIdentity(peerId)
      return 'TrUeB2';
    }
    ...
    receiveSubscription({senderId, requestId}) {
      console.log("Handling new subscription", senderId, requestId);
    ...
    assignNewSiteId(peerId) {
      ...
      const siteAssignmentMessage = new Messages.DM.Portal.Update.SiteAssignment();
      ...
      const portalMessage = new Messages.DM.Portal();
      portalMessage.setUpdate(updateMessage);
      const dm = new Messages.DM();
      dm.setPortal(portalMessage);

      const channelId = `/portals/${this.id}`;
      console.log("Sending notification to ", channelId);
      this.decNet.notify({
        channelId,
        portalId: this.id,
        hostId: this.hostPeerId,
        body: dm.serializeBinary()
      });
    }

    sendSubscriptionResponse(requestId) {
      const response = new Messages.DM.Portal.SubscriptionResponse();
      ...
      this.tethersByFollowerId.forEach((tether, followerId) => {
        const tetherMessage = new Messages.DM.Portal.Tether();
      ...
      const portalMessage = new Messages.DM.Portal();
      portalMessage.setSubscriptionResponse(response);
      const dm = new Messages.DM();
      dm.setPortal(portalMessage);
      this.decNet.respond({requestId, ok: true, body: dm.serializeBinary()})
    }

    async receiveUpdate({senderId, body}) {
      const dm = Messages.DM.deserializeBinary(body);
      const updateMessage = dm.getPortal().getUpdate();
      ...
      } else if (updateMessage.hasTether()) {
        this.receiveTetherUpdate(updateMessage.getTether())
      } else {
    ...
    setFollowState(newState, newLeaderId) {
      ...
      if (oldState !== newState || oldLeaderId !== newLeaderId) {
        ...
        const updateMessage = new Messages.DM.Portal.Update();
        updateMessage.setTether(tetherMessage);
        const portalMessage = new Messages.DM.Portal();
        portalMessage.setUpdate(updateMessage);
        const dm = new Messages.DM();
        dm.setPortal(portalMessage);
    ...
    broadcastEditorProxyCreation(editorProxy) {
      const editorProxyCreationMessage = new Messages.DM.Portal.Update.EditorProxyCreation();
    ...
    broadcastEditorProxySwitch(editorProxy) {
      const editorProxySwitchMessage = new Messages.DM.Portal.Update.EditorProxySwitch();
