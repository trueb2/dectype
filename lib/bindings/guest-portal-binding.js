...
const FollowState = require('../net/follow-state');
const BufferBinding = require('./buffer-binding');
const EditorBinding = require('./editor-binding');
const SitePositionsComponent = require('../components/site-positions-component');
const {getEditorURI} = require('../uri-helpers');
const NOOP = () => {};

module.exports =
  class GuestPortalBinding {
    constructor ({decNet, portalNumber, remoteUserId, workspace, notificationManager, didDispose}) {
      this.decNet = decNet;
    ...
    async initialize () {
      try {
        this.portal = await this.decNet.joinPortal(this.remoteUserId, this.portalNumber);
    ...
    dispose () {
      // TODO notify host about leaving.
    ...
    siteDidJoin (siteId) {
      const hostLogin = 'trueb2'; //= this.portal.getSiteIdentity(1);
      const siteLogin = 'not_trueb2'; //{login: siteLogin} = this.portal.getSiteIdentity(siteId);
    ...
    // getRemoteEditors () {
    //   const hostIdentity = 'trueb2'; //this.portal.getSiteIdentity(1);
    ...
    //
    //     remoteEditors.push({
    //       hostGitHubUsername: hostIdentity,
