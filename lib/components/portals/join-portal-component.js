...
module.exports =
  class JoinPortalComponent {
    ...
    readAfterUpdate() {
      ...
      if (!previousPortalIdEditor && this.portalIdEditor) {
        this.portalIdEditor.onDidChange(() => {
          ...
          this.refs.joinButton.disabled = !this.checkPortalId(portalId);
        })
      }
    }
    ...
    async showPrompt() {
      ...
      if (this.checkPortalId(clipboardText)) {
    ...
    async joinPortal() {
      ...
      const portalId = this.checkPortalId(this.refs.portalIdEditor.getText().trim());
    ...
    checkPortalId(portalId) {
      // TODO: check for valid identifier or alias like alias_trueb2:USERID_1234
      if (portalId.split(":").length !== 2) {
        return null;
      }
      return portalId;
    }
